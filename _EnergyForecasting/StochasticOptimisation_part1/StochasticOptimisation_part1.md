...
Deciding how to operate a battery, when to charge and when to discharge, depends on future prices, demand, and generation, none of which are known in advance. This is exactly the setting stochastic optimisation is built for.

## 2. Deterministic Optimisation

Deterministic optimisation is the standard case: every input to the problem, prices, demand, generation, is treated as known in advance, and the task is to find the single best sequence of decisions given that fixed information. This is the same logic behind classical optimisation methods such as linear programming and dynamic programming, the kind of problems covered in a classical optimisation curriculum, and it also matches the structure of classic inventory control problems: a resource is held in stock, and decisions are made period by period to use it optimally.
...
- Wimmeder, S. (2021). *Stochastic Optimization of a Battery Storage System*. Master's thesis.
