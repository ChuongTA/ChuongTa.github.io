"""
Plotting functions for the bootstrap-residual wave forecasting pipeline.
Structurally identical to the QR/QRF plotting module in ../4_QR_QRF_Probabilistic,
just with a distinct color for the single "Bootstrap" model instead of QR/QRF.
"""
import os
import numpy as np
import matplotlib
...
def plot_split_diagram(fold_boundaries, out_path):
    ...
    for i, fold in enumerate(fold_boundaries):
        y = len(fold_boundaries) - i
        segments = [
            ("Train", fold["train_start"], fold["train_end"]),
            ("Validate", fold["valid_start"], fold["valid_end"]),
            ("Test", fold["test_start"], fold["test_end"]),
        ]
        for label, start, end in segments:
            width_days = (end - start).total_seconds() / 86400
            ax.barh(y, width_days, left=start, height=0.6, color=colors[label],
    ...
    ax.set_title("Walk-Forward Train / Validate / Test Split (Wave Energy, ERA5)", fontsize=13,
...
def plot_forecast_fan(dates, y_true, preds_by_model, lead_time_hours, fold_label, out_path, ylabel="Value"):
    fig, axes = plt.subplots(1, len(preds_by_model), figsize=(8 * len(preds_by_model), 5.5), sharey=True)
    if len(preds_by_model) == 1:
        axes = [axes]
    colors = {"Bootstrap": "#7B1FA2", "QR": "#1976D2", "QRF": "#00796B"}
    ...
    for ax, (name, preds) in zip(axes, preds_by_model.items()):
        ...
        ax.plot(dates, y_true, color="#E91E63", lw=1.4, label="Actual", zorder=4)
    ...
    axes[0].set_ylabel(ylabel, fontsize=11, fontweight="bold", color="#37474F")
    plt.tight_layout()
...
def plot_reliability_diagram(reliability_rows, out_path, title_suffix=""):
    fig, ax = plt.subplots(figsize=(4.5, 4.5))
    ax.plot([0, 1], [0, 1], color="#37474F", linestyle="--", lw=1.5, alpha=0.7,
            label="Perfect Calibration (Ideal)")

    colors = {1: "#2979FF", 3: "#00E676", 6: "#FFC400", 12: "#FF9100", 24: "#FF1744", 48: "#AA00FF"}
    line_style_cycle = ["-", "--", ":", "-."]
    ...
    for lt in lead_times:
        for name in models:
            ...
            ax.plot(noms, covs, marker="o", markersize=4, lw=1.6, linestyle=line_styles[name],
                    color=colors.get(lt, "#000000"), label=f"{name}, {lt}h")

    ax.set_xlabel("Nominal Coverage", fontsize=10, fontweight="bold", color="#37474F")
    ax.set_ylabel("Empirical Coverage (avg. across folds)", fontsize=10, fontweight="bold", color="#37474F")
    ax.set_title(f"Reliability Diagram {title_suffix}\nAveraged Across Walk-Forward Folds", fontsize=11,
                 fontweight="bold", pad=10, color="#263238")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.spines["left"].set_color("#CFD8DC")
    ax.spines["bottom"].set_color("#CFD8DC")
    ax.legend(fontsize=6.5, loc="lower right", frameon=True, facecolor="white", edgecolor="#ECEFF1", ncol=2)
    ax.grid(True, alpha=0.2, linestyle="--", color="#90A4AE")
