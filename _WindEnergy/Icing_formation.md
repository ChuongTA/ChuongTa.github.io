...
![Average wind speed in the world in winter [1](https://climate.copernicus.eu/esotc/2022/wind-solar-energy-resources)](/files/Icing_formation/Wind_speed_winter_in_europe.png)

![Countries by january average low temperature [2](https://www.reddit.com/r/europe/comments/kob7to/countries_by_january_average_low_temperatures_of/)](/files/Icing_formation/temperature_in_january.jpg)
...
![Median volumetric diameter.[4](https://pesticidestewardship.org/pesticide-drift/understanding-droplet-size/)](/files/Icing_formation/vmd-ncsu.gif)
...
**Heat Losses:**
* **Convective heat transfer ($$Q_{conv} = q_c (T_a - T_s)$$):** Cold air rushing over the wing removes heat. The heat transfer coefficient $$q_c$$ determines how efficiently this occurs.
* **Radiative cooling ($$Q_{rad} = -\epsilon \sigma (T_s^4 - T_a^4)$$):** The surface radiates heat to the cold sky, following the Stefan-Boltzmann law. While usually small, this becomes more significant at high altitudes.
* **Evaporative cooling ($$Q_{evap} = -\dot{m}_s L_s$$):** Water molecules can evaporate or sublimate directly from the ice, carrying away significant energy—approximately 2.83 MJ/kg.

**Heat Sources:**
* **Latent heat from freezing ($$Q_{lat} = \dot{m}_{freeze} L_f$$):** When water freezes, it releases energy (about 3.34 MJ/kg). On the blade surface, this released heat actually opposes further freezing.
* **Sensible heat from droplets ($$Q_{imp} = m_{imp} c_{p,w} (T_a - T_s)$$):** Incoming droplets carry thermal energy based on their temperature. If they are warmer than the surface, they deposit heat when they impact.

---

#### Makkonen Model
The Makkonen model directly calculates the ice mass accumulation rate over time.

**Mass Accumulation Equation:**
$$\frac{dM}{dt} = \alpha_1 \alpha_2 \alpha_3 \omega v A$$

* **Collision efficiency ($$\alpha_1$$):** Not all droplets in the air hit the object; small droplets follow air streamlines better and may miss the object, while large droplets have more inertia and collide.
* **Sticking efficiency ($$\alpha_2$$):** This accounts for droplets that hit the surface but bounce off or shatter instead of sticking, depending on impact velocity and size.
* **Accretion efficiency ($$\alpha_3$$):** Of the water that sticks, only a fraction freezes; the rest may run off as liquid. This is determined by the freezing fraction:
  $$\alpha_3 = \frac{c_{p,w} (T_f - T_d) + L_f}{L_f + L_s - c_{p,i} T_s}$$
  This equation demonstrates how droplet temperature and surface conditions determine the final freezing fraction.
* **Mass flux ($$\omega v A$$):** This is the rate at which water would flow through the cross-sectional area if no object were present (liquid water content $\times$ velocity $\times$ area).
...
![Glaze Ice [6](https://en.wikipedia.org/wiki/Glaze_%28ice%29)](/files/Icing_formation/glaze_ice.jpg)
...
- Structural: Added mass of ice (up to 50% of blade weight). Uneven ice accretion and mass imbalance. Also, it increases Loads and Fatigue: Higher static and dynamic loads; reduced fatigue lifetime on components. Vibrations: Excessive vibrations due to rotor unbalance36363636. Reduced Natural Frequency: Natural frequencies decrease, risking resonance.
- Operational/Safety: Ice accumulation on sensors38. Ice chunks detaching (shedding)
Technical Unavailability: WT stoppage, downtime, and reduced availability. Safety Hazard: Ice throw/fall risks to personnel and nearby asset

![Icing formation in wind turbine consequences [7]((https://www.aere.iastate.edu/~huhui/WT-icing.html))](/files/Icing_formation/WT_icing_consequences.jpg)

### 5. De-icing methods
The de-icing methods are described in the table below: 

### Wind Turbine De-icing and Anti-icing Methods
