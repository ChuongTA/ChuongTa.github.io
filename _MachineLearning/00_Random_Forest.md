...
Multiple training datasets are generated from the original data using **bootstrap sampling**. Each tree $b$ is trained on its own sample $\mathcal{D}^{*b}$:
...
Each tree is grown to its maximum depth without pruning. This intentionally allows individual trees to overfit, which is later corrected by averaging.
...
$$\hat{y}_{\text{RF}}(\mathbf{x}) = \arg\max_{k} \sum_{b=1}^{B} \mathbb{1}(\hat{f}^{*b}(\mathbf{x}) = k) \tag{3.15}$$
...
* **Feature Importance:** Naturally calculates which variables are most important for the model.
* **Parallelization:** Trees can be trained simultaneously across CPU cores.

### Limitations
* **Interpretability:** Harder to interpret than a single tree (often called a "Black Box").
* **Memory Usage:** Deep forests can consume significant RAM in production environments.

---

## 5. Conclusion

Random Forest is a reliable "go-to" algorithm for most tabular datasets. While it lacks the simple visual rules of a single tree, its stability and accuracy make it a favorite for industrial applications.
