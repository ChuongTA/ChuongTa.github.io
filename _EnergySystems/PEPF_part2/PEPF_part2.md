...
Each row needs calendar features (hour, day of week, month) and lagged price features. The lags are built relative to the forecast lead time $L$, so a model trained for the 6h-ahead task only ever sees information that would genuinely be available 6 hours before delivery:
...
**Two of the four folds are consistently the hardest to calibrate, for both models.** Breaking the average down by fold: Fold 1 (test window 19 September to 3 October 2024) and Fold 3 (30 March to 13 April 2025) calibrate well, close to nominal coverage at every lead time. Fold 2 (24 December 2024 to 7 January 2025, spanning Christmas and New Year) and Fold 4 (4 to 18 July 2025) calibrate noticeably worse, sometimes dropping to 53 to 66% actual coverage against an 80% nominal target for both QR and QRF. A plausible explanation for Fold 2: the calendar features (`is_weekend`, `dayofweek`, `is_peak`) encode a normal working week, and public holidays break that pattern in ways the model hasn't seen enough of. This is exactly the kind of failure mode walk-forward validation is designed to catch and a single split would have hidden.

**Quantile crossing for QR is still real, and still lines up with the worst-calibrated folds.** Averaged across folds it ranges from under 1% at 1h ahead to over 20% at 6h ahead in the worst fold, and it consistently spikes in the same folds where coverage collapses. QRF has no such issue by construction.
...
**Practical takeaway.** QRF remains the stronger model of the two, and walk-forward validation shows that conclusion holds up across different seasons and market regimes, not just the one three-month window the original single split happened to test on. The more important finding from moving to walk-forward CV isn't which model wins, it's that **both models' calibration is regime-dependent**, and any deployment of this pipeline should expect noticeably worse-calibrated intervals during holiday periods until the feature set explicitly accounts for them.
